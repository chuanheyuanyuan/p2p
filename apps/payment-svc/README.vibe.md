# Payment Service · README.vibe.md

> Domain: 放款与还款网关

## ⚡ Quickstart
1. `task run:payment-svc` — 启动本服务（若 `app/main.py` 尚未创建会给出提示）。
2. `task lint` / `task test` — 统一代码质量与测试（基于 ruff + pytest）。
3. 使用 VS Code REST Client 打开 `sample.http`，即可在 vibe coding 中快速回放接口。

## 🔌 API 快速体验
- 默认本地地址：`http://localhost:8007`
- 推荐带上 `X-Request-Id` 方便链路追踪。
- 事件钩子：DISBURSED, REPAYMENT_POSTED, PAYMENT_CALLBACK_TIMEOUT。
